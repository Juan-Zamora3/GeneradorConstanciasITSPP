...
export default function ImprimirConstancias() {
  ...
  const resolveFieldValue = (key, c) => {
    ...
    // Si viene un key custom, intenta tomarlo directo del objeto constancia
    return (map[key] ?? c?.[key] ?? '');
  };

  // Dibuja un texto dentro de un rectángulo definido en %
  const drawTextInRectPct = (page, font, boldFont, campo, texto) => {
    const { width: pw, height: ph } = page.getSize();
    const x = (campo.xPct || 0) * pw;
    const yTop = (campo.yPct || 0) * ph;      // y medido desde arriba (editor)
    ...
    // PDF tiene origen abajo-izquierda; convertimos y (desde arriba) a baseline aproximada
  ...
  // ---------- Cargar config de constancia desde el curso ----------
  useEffect(() => {
    (async () => {
      try {
        const conf = await getConfigConstancia(cursoId);
        setCfg(conf); // { plantilla:{url,...}, campos:[...] }
      } catch (e) {
        console.error('No se pudo cargar la config de constancia:', e);
        setCfg(null);
      }
    })();
  }, [cursoId]);

  // ---------- Generación por constancia usando plantilla + campos ----------
  const generarPDFConstancia = async (constancia) => {
    try {
      let pdfDoc;
      if (cfg?.plantilla?.url) {
        // Cargar PDF base desde Storage (asegúrate de tener CORS del bucket configurado)
        const ab = await fetch(cfg.plantilla.url).then(r => r.arrayBuffer());
      ...
      // Pintar cada campo en su página correspondiente
  ...
  // ---------- Generar todas + subir a Storage y registrar en Firestore ----------
  const generarTodasLasConstancias = async () => {
    ...
    for (let i = 0; i < constancias.length; i++) {
      ...
      if (pdfBytes) {
        // Subir a Storage + registrar subcolección (Cursos/{cursoId}/Constancias)
        try {
          const participante = {
            nombre: c.nombre,
            correo: c.correo,
          ...
          generadas.push({
            ...c,
            pdfBytes,
            pdfUrl: url // url pública de descarga
          });
        } catch (e) {
          console.error('No se pudo subir/registrar la constancia:', e);
          // Aun así conserva para zip local
          generadas.push({
            ...c,
            pdfBytes,
  ...
  // ---------- Entradas requeridas y arranque ----------
  useEffect(() => {
  ...
  useEffect(() => {
    if (cfg && constancias && equipo && curso && factura) {
      generarTodasLasConstancias();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cfg]);

  // ---------- Contador regreso automático ----------
  useEffect(() => {
      ...
      {impresionCompleta && (
        <div className="absolute inset-0 pointer-events-none">
          {Array.from({ length: 20 }).map((_, i) => (
            ...
            >
              {i % 3 === 0 ? <Star className="w-4 h-4 text-yellow-400" /> :
               i % 3 === 1 ? <Sparkles className="w-4 h-4 text-purple-400" /> :
                ...
                {generandoPDFs || !impresionCompleta ? (
                  <>
                    ...
                    <div className="space-y-6">
                      <motion.h1 className="text-4xl bg-gradient-to-r from-indigo-900 to-purple-700 bg-clip-text text-transparent"
                      ...
                      <motion.p className="text-gray-600 text-xl" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }}>
                        {generandoPDFs ? 'Preparando documentos con la plantilla y posiciones guardadas…' : 'Imprimiendo tus constancias…'}
                      </motion.p>
                    </div>

                    <motion.div className="space-y-6" initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.7 }}>
                      ...
                      <div className="relative">
                        ...
                        <motion.div
                          ...
                          animate={{ boxShadow: ["0 0 0 rgba(99,102,241,.4)", "0 0 20px rgba(99,102,241,.4)", "0 0 0 rgba(99,102,241,.4)"] }}
                          transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                        />
                      </div>
                    </motion.div>

                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.6, delay: 0.9 }}>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              ...
                              <div className="text-left">
                                ...
                                <p className="text-indigo-600">{(curso?.cursoNombre || curso?.nombre) ?? ''} - {equipo?.nombre ?? ''}</p>
                              </div>
                            </div>
                            <motion.div className="w-4 h-4 bg-gradient-to-r from-green-400 to-emerald-400 rounded-full"
                              animate={{ scale: [1, 1.5, 1], opacity: [0.5, 1, 0.5] }} transition={{ duration: 1, repeat: Infinity, ease: "easeInOut" }} />
                          </div>
                        </div>
                      </div>
                    </motion.div>

                    <motion.div className="space-y-4" initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 1.1 }}>
                      <h3 className="text-indigo-900 text-xl font-medium">Constancias en proceso:</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {constancias.map((c, index) => {
                          const doneCount = Math.ceil((progreso / 100) * constancias.length);
                          ...
                          return (
                            <motion.div
                              ...
                              initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.6, delay: 1.3 + index * 0.05 }}
                            >
                              <div className="flex items-center space-x-3">
                                <div className={`w-3 h-3 rounded-full ${
                                  isCompleted ? 'bg-gradient-to-r from-green-400 to-emerald-400'
                                              : isCurrent ? 'bg-gradient-to-r from-indigo-400 to-purple-400 animate-pulse'
                                                          : 'bg-gray-300'
                                }`} />
                                <span className={`${isCompleted ? 'text-green-700' : isCurrent ? 'text-indigo-700' : 'text-gray-500'} font-medium`}>
                                  {c.nombre}
                                </span>
                              </div>
                              {isCompleted && <CheckCircle className="w-5 h-5 text-green-600" />}
                              {isCurrent && !isCompleted && (
                                <motion.div className="w-5 h-5 border-2 border-indigo-600 border-t-transparent rounded-full"
                  ...
                  <>
                    ...
                    <div className="space-y-6">
                      <motion.h1 className="text-4xl bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent"
                        initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.3 }}>
                        ¡Impresión Completada!
                      </motion.h1>
                      <motion.p className="text-gray-600 text-xl"
                        initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }}>
                        Se han generado y subido {constanciasGeneradas.length} constancia{constanciasGeneradas.length !== 1 ? 's' : ''}.
                      </motion.p>
                    </div>

                    <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.7 }}>
                      <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-green-200 shadow-lg rounded-xl">
                        <div className="p-8">
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {constanciasGeneradas.map((c, index) => (
                                <motion.div key={index} className="flex items-center justify-between space-x-3 p-3 bg-white/50 rounded-lg"
                                  initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.9 + index * 0.05 }}>
                            ...
                            <div className="mt-6">
                              <button
                                ...
                                className="px-6 py-3 rounded-lg bg-green-600 hover:bg-green-700 text-white font-medium shadow"
                              >
                                Descargar ZIP
                              </button>
                            </div>
                    ...
                    <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.95 }}
                      initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 1.0 }}>
                      <button
